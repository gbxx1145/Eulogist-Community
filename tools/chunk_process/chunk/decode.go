...
func NetworkDecode(air uint32, data *bytes.Buffer, count int, r cube.Range) (*Chunk, error) {
	var (
		c   = New(air, r)
		err error
	)
	...
	for i := 0; i < len(c.sub); i++ {
		b, err := decodePalettedStorage(data, NetworkEncoding, BiomePaletteEncoding)
		if err != nil {
			return nil, err
		}
		if b == nil {
